...
def positive_sentiment(df):
    ...
    print("Entering")
    st.pyplot(sentiment_analysis(df, start_date, end_date))
    print("Exiting")
...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show the Sentiment"):
        if start_date >= end_date:
            st.error("Please select a valid date range. The start date should be before or equal to the end date.")
        else:
