...
tokenizer = []
le = []
with open(r'D:\Projects\WhatsApp Analyzer\Exploratory-Data-Analysis-and-Emotion-Detection-Using-LSTM\Emotion Detection\LSTM_BiLSTM\tokenizer.pickle', 'rb') as f:
    tokenizer = pickle.load(f)
    
with open(r'D:\Projects\WhatsApp Analyzer\Exploratory-Data-Analysis-and-Emotion-Detection-Using-LSTM\Emotion Detection\LSTM_BiLSTM\labelEncoder.pickle', 'rb') as f:
    le = pickle.load(f)

with open(r'D:\Projects\WhatsApp Analyzer\Exploratory-Data-Analysis-and-Emotion-Detection-Using-LSTM\Emotion Detection\LSTM_BiLSTM\model.pickle', 'rb') as f:
    model = pickle.load(f)
...
def fetch_stats(df):
    ...
    print(f"Total messages : {return_obj['total_messages']}")
    print(f"Total Words : {return_obj['total_words']}")
    print(f"Total Links : {return_obj['link_shared']}")
    print(f"Total Media : {return_obj['media_message']}")
...
def sentiment_analysis(df: pd.DataFrame, start_date, end_date):
    """
    ...
    cloud_df = df[(df['date'] >= start_date) & (df['date'] <= end_date)]
    if cloud_df.empty:
        plt.text(0.5, 0.5, 'No data is present', fontsize=14, ha='center')
        return plt.gcf()
    else :
        grouped = cloud_df.groupby('name')

        # Create a dictionary to store messages for each user
        user_messages = {}

        # Alternatively, you can use a dictionary comprehension
        user_messages = {name: group['message'].tolist() for name, group in grouped}

        # Now, user_messages contains messages for each user
        print(type(user_messages.keys()))
        usernames = list(user_messages.keys())
        updated_user_message = {}
        delimiter = " "
        for key in user_messages.keys():
            updated_user_message[f"{key}"] = delimiter.join(user_messages[key])
        
        user_sentiments = {}

        for key in updated_user_message.keys() :
            user_sentiments[f"{key}"] = predict(updated_user_message[key])
        
        print(user_sentiments)
        
        sentiment_df = pd.DataFrame(user_sentiments)
        print(sentiment_df.head())

        sentiment_score = sentiment_df.iloc[0:1,:].values.tolist()
        emotions = sentiment_df.iloc[1:,:].values.tolist()
        print(sentiment_score)
        print(emotions)

        for i in range(len(emotions[0])) :
            emotions[0][i] = f"{usernames[i]}-{emotions[0][i]}"
        
        plt.bar(emotions[0], sentiment_score[0], color="skyblue", width=0.8)
        fig = plt.gcf()
        return fig
        # cloud_df['sentiment'], cloud_df["sentiment_score"] = cloud_df.message.apply(lambda text: predict(text))
        # cloud_df['sentiment'], cloud_df["sentiment_score"] = predict(message)
        # print(cloud_df.head())
        # sentiment = cloud_df[['name', 'sentiment_score']].groupby('name').mean()
        # s_a = sentiment.sort_values('sentiment_score', ascending=False).head(15)
